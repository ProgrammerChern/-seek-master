...
Page({
    ...
    bindNumber: function(a) {
        this.setData({
            areanum:a.detail.value
        });
    },
    subtract: function() {
        var current = this.data.areanum;
        this.setData({
            areanum:current > 0 ? --current : 0
        });
    },
    add: function() {
        var current = this.data.areanum;
    ...
    selected: function(a) {
        var e = this,goods=e.data.goods,type=e.data.type.goods, index = a.currentTarget.dataset.index, item = a.currentTarget.dataset.item;
        if (!goods){
            goods=[];
            goods.push(item.title);
        } else {
            if (typeof goods == "string") {
                goods = goods.split(",");
            }
            var idx = goods.indexOf(item.title);
            if (idx != -1) {
                goods.splice(idx, 1);
            } else {
                goods.push(item.title);
    ...
    typeselected: function(a) {
        var e = this,goods=e.data.types,type=e.data.type.type, index = a.currentTarget.dataset.index, item = a.currentTarget.dataset.item;
        if (!goods){
            goods=[];
            goods.push(item.title);
        } else {
    ...
    gbindNumber: function(a) {
        var e = this,type=e.data.type.goods, index = a.currentTarget.dataset.index;
        if (a.detail.value == 0 ) {
            a.detail.value = 1;
        }
